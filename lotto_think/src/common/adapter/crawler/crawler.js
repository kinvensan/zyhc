...
module.exports = class extends EventEmitter {
  constructor(config) {
    ...
    const options = helper.extend(defaultOptions, config);
    super(options);
  }
  get queueSize() {
  ...
  init() {
    ...
    // return defaultOptions with overriden properties from options.
    self.options = helper.extend(defaultOptions, options);
  ...
  queue(options) {
    ...
    options = helper.isArray(options) ? options : [options];
    ...
    for (var i = 0; i < options.length; ++i) {
      ...
      self._pushToQueue(
        helper.isString(options[i]) ? {uri: options[i]} : options[i]
      );
    }
  }
